...
//						}
			...
			function getTokenAndStatus () {
				...
				if (tokenExpiresOn < now + 600) {
					...
					getJSON('<?= ($baseAppPath) ?>/teams/token', null, function(err, data) {
						if (err !== null) {
			...
			function checkStatus (token) {
				...
				if (token != null) {
					getJSON('https://graph.microsoft.com/beta/me/presence', token, function(err, data) {
						if (err !== null) {
							...
							} else if (data.availability == 'Away') {
